<?php
/**
 * Feature Name:    Comment Functions for 30 Second WordPress Theme-Theme
 * Version:         1
 * Author:          Rene Reimann
 ...
 * @return void
 */
function t30_the_comment( $comment, Array $args = array(), $depth = 0 ) {
	?>
		...
		<div class="comment_container">
			...
			<?php if ( $comment->comment_approved === '0' ) : ?>
				<p class="comment-awaiting-moderation alert-info"><?php
					_e( 'Your comment is awaiting moderation.', 't30' );
					?></p>
			<?php endif; ?>
 ...
 * @return  int
 */
function t30_get_count_pings( $post_id = NULL ) {
	global $wp_query;
 ...
 * @return  void
 */
function t30_the_pings( $comment ) {

	$url	    = esc_url( $comment->comment_author_url );
	$icon_args  = array( 'url' => $url );
	$icon	    = t30_get_external_favicon( $icon_args );
 ...
 * @return  string
 */
function t30_get_external_favicon( Array $args = array()  ) {
	...
	$rtn = apply_filters( 'pre_t30_get_external_favicon', FALSE, $args, $default_args );
	if ( $rtn !== FALSE )
		return $rtn;

	$args = wp_parse_args( $args, $default_args );
	$args = apply_filters( 't30_get_external_favicon_args', $args );
	...
	return apply_filters( 't30_get_external_favicon', $output, $args );
}